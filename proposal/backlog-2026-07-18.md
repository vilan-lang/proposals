...
4. **Sub-file incremental parsing** (L; **recommended CLOSE-BY-MEASUREMENT
   2026-07-22, user call pending**) — the item's motivation was chumsky-era
   parse cost; post-H6, parsing is **3.7% of a compile** (callgrind, S5) and
   the per-keystroke floor is `build()` + whole-program checks (the E3
   Phase-2 measurements) — an L-sized tree-reuse engine would optimize a ~3%
   slice. Joins E3 Phase 3 in the take-when-it-hurts drawer: revisit only if
   a real project's single-file parse time becomes observable in the editor.
