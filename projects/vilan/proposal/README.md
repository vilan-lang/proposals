...
| `tracker-migration.md` | PREPARED 2026-08-28 (Order 18, cycle 36); execution serial, between cycles | Backlog N17 — the full-repo per-item tracker migration runbook: wholesale cutover to one `projects/vilan/tracker/`, IDs carried verbatim as filenames (~2,800 citations stay resolvable with zero rewrites), the chronicle to its own file, freeze-and-chain for the archive, split/verify scripts prepared and tested against scratch copies. |
| `lifetimes.md` | DRAFT 2026-08-28 (the lifetime-model session) — five owner questions | The lifetime model whole: why implicit lifetimes hit cliffs not exponentials and vilan stands clear of every cliff; the measured census (7.1% aliased, ~93% unique, copy-heavy 30:1); the reactive graph's five repairs (owner spine proven acyclic); last-use disposal priced (loans extend via view origins, branch-join drop specialization, the extern retention contract); Tier B = Perceus-RC + generational refs per corner, JS keeps the host GC; capture semantics specified as shipped. Reopens claims-and-epochs' "C4 is the last major change", by name. |
