...
## 11. As built — Order 36 (2026-09-14, lane lang-a-36): A99, A98, B334

**A99 — the parent methods retired, as ruled (hard removal, the `bind_each` trio included).** `View::when`, `View::swap`, `View::swap_split`, `View::bind_each`, `View::bind_each_values`, `View::bind_each_by` are deleted from the browser layer and the five the process twin had (it never had `swap_split`) from `process/ui.vl`; the value forms are the only forms. `split_route` — the third name §10's shadowing hazard forced — is folded back into the free `swap_split`, and `chunks.rs` recognizes the free `swap` only (its `view_method` arm is gone; the retarget table is one entry). The `std::web` prelude carries `when`, `swap`, `each`, `each_values`, `each_by` beside `View`/`view`, so a child hole spells them bare. A call to a retired method earns the `MethodLookup::NoMethod` message's fifth suffix (row 212 re-keyed: "`bind_each` is no longer a `View` method (A99): write `child(each(..))` to place it at this element's end, or `{each(..)}` in a child hole to place it there") and the editor's one new arm, `Document::retired_slot_method_fix`, rewrites the call node to `child(each(…))` in place. The estate — docs (9 pages), examples (9 sites), the corpus's `ssr-render.vl`, vilan-core tests (7 files), vilan-cli tests (6 files), and at the seal the wasm playground's router fixture — is rewritten; kolt's nine sites are the owner's. **One thing the rewrite is not: type-preserving for `each_by`** — the retired METHOD bound `T` through the receiver path and the free call does not, so an unannotated render closure over `each_by` freezes `T` abstract (B347, pre-existing, four sites annotated).

**A98 — the region reconciler leaves an unmoved row alone.** `settled_steps` decides which rows already stand in their new relative order (two greedy scans, forward and backward, the better wins — LIS-equal on append, prepend, removal at either end, a value refresh, a single move either way and an adjacent swap, all pinned; a patience sort is the one function to replace if a genuine reshuffle ever matters), `row_references` gives every moved row the marker it is threaded in before, and `Region::open_row_before` places it. One thousand rows between a header and a footer: append and prepend cut 1000 → 0, a removal, a value change or a single move cut 1000 → 1, a reverse 999 (the control). Both twins take the change; the pin asserts the cost AND the resulting tree per edit.

**B334 — the free function wins a receiverless call (R4).** `Analyzer::receiverless_call_subject`, consulted by `resolve_prepped_local` before the ordinary scope walk, so the rule is order-independent; narrow three ways — call subjects only, only when the impl body's own declaration takes a receiver, resolved from the impl body scope's parent. §3b's "sugar" is now literal, and the std exhibit is gone with the methods.
