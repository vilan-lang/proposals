...
### 9.7 A `[service]` as a plain HTTP API (A120 — the connectionless leg gets a client; paper, Order 39)

*Written by lane papers-39 of Order 39 and spliced here at the sweep (2026-09-22), directly after §9.6c, because it is the last thing §9's transport story leaves
...
*Sweep notes (2026-09-22): §9.7.7.1's crash is tracker B374; §9.7.8's hole (`client = H` compiles clean) is A120 S2's refusal; the `export impl` find is B375; the JSON-reader find landed as std-39's add-on 2321790e (S1 below is therefore BUILT); the long-argument-list half is B383; the content-type posture (§9.7.9) is A120's Q3.*

#### 9.7.1 The ask, and the answer up front
...
table, and one bug.** That is §9.7.4–§9.7.8, and the bug (§9.7.8) is the only part that
must land before kolt's login is safe to expose.

#### 9.7.2 The premise check
...
- *"…and a story for a service that is ONLY that"* — **true**, and that story
  is §9.7.5 (what such a service may not declare) and §9.7.6 (how it is gated).
...
#### 9.7.3 What the POST leg answers today — measured
...
| `echo` with NO arguments | **200** | **`{"Success":NaN}`** ← §9.7.8 |
...
2. **An application error is inside `Success`.** `Result<AccountToken, str>`'s
   ...
   way. This is right, and §9.7.7 keeps it.
...
4. **`__attach` is not**, and says so in one sentence — which is exactly the
   boundary §9.7.5 draws: mirrors need a connection.
...
#### 9.7.4 `over_http` — the constructor
...
(measured, §9.7.1), so a caller who wants it asks for it. The docs sentence is
...
**It is generated only for a service an HTTP client can actually hold** — §9.7.5.

#### 9.7.5 What is refused, and where
...
- An **`[expose]`d field** puts a `RemoteSource<T>`/`KeyedSource<K, T>` field on
  ...
  POST answers `unknown connection` (§9.7.3). Refused structurally.
- A **handle-returning method** puts a `reactive: ReactiveClient` field on the
  ...
  for a hand-built protocol, and it still fires (§9.7.3).
...
#### 9.7.6 `authorize`, and the login that mints the socket's token
...
(§9.7.3). That is deliberate and documented at the site — the SSE and POST legs
...
Today's answer is "200 for everything the protocol decides" (§9.7.3), and the
...
##### 9.7.7.1 The crash that must go with it
...
#### 9.7.8 The hole an arbitrary caller walks through — and why it blocks the item

`{"method":"echo","args":[]}` answered `{"Success":NaN}` (§9.7.3). That is not an
...
#### 9.7.9 CSRF and same-origin posture
...
construction. The hazard arrives the moment §9.7.6's `authorize_request` exists and
...
- **CORS stays the app's.** `on_request` and `Response::builder().set_header`
  ...
  carries the three-line recipe. **Q3 in §9.7.12.**

#### 9.7.10 kolt's three sites, rewritten

This is the worked example the item asks for. Nothing below needs anything from
§9.7.5–§9.7.9 except §9.7.4's constructor and §9.7.8's arity gate.
...
module-level by loan. `http` is §9.7.5's opt-in marker — this service declares no
...
happen", the inner is "what did the server decide", and §9.7.7's status rule is that
...
The route's arity check, and the JSON reader poisoning on a type mismatch (§9.7.8).
...
**Exit:** a service-layer pin per row of §9.7.8's table — a short list, a string
...
§9.7.4's constructor, generated under §9.7.5's rule; the `client = H` case not
generated; `[service(http)]` as §9.7.5's opt-in marker with its three refusals
...
§9.7.7's arm-to-status map; the 501 as an envelope; 405 on a non-POST;
`HttpTransport::call` reading the status and wrapping the `fetch` rejection
(§9.7.7.1); `Content-Type` set on the request and required by the leg (§9.7.9).

**Exit:** one e2e pin per row of §9.7.7's table, asserting the status AND that a
...
§9.7.6's second hook, its `Session` on the `RpcRequest`, and §9.7.6's four-row
...
§9.7.10, at the owner's word, after S1+S2.
...
#### 9.7.12 Open questions, each with a recommendation

**Q1 — `[service(http)]`: a marker, a mode, or nothing?**

> **Rec: an opt-in MARKER (§9.7.5).** The structural refusal already exists and is
...
> **Rec: a SECOND hook (§9.7.6).** `Handshake::token()` reads the `"token."`
...
> **Rec: NO (§9.7.9).** `on_request` plus `set_header` is already the whole
...
> **Rec: `over_http` (§9.7.4).** `connect` means "there is now a connection", and
...
> **Rec: NEVER (§9.7.7).** `Result<AccountToken, str>`'s `Err` arm is a value that
...
> fact §9.7.6's gate reports. kolt's hand-written leg already answers 200 for both
...
owner disagrees: `over_http` makes **no call** (§9.7.4 — A92's precedent, and a
login form constructs a client per mount), and the **envelope and the status
never disagree** (§9.7.7 — which is what makes S3 non-breaking for every vilan
client, measured).

#### 9.7.13 What this does not do

- **Mirrors, handles and the reverse direction over HTTP.** They need a
  connection by construction (§9.7.5), and the leg that has one is the socket.
...
- **OpenAPI, or any schema export.** The contract hash is the version check
  (§9.7.3, §9.7.4) and it is opaque by design. A readable schema is worth wanting the
...
- **B363.** §9.6c's `Option<RpcError>` stands on "the language has no unit
  ...
  return type is that paper's to move, not this one's; nothing in §9.7.1–§9.7.12 depends
