...
| [B255](items/B255.md) | NEW — `bind_each`/`bind_each_by` keep the LIVE list as `row_items` (`row_items.write() = list`, and the effect's `list` is the cell's storage), so an in-place `remove`/`insert` through `SignalCell::update` shrinks it under `row_keys` and `reconcile` panics at `old_items[index]` (kolt channel.vl:54: `index out of bounds: the length is N but the index is N`); an in-place element write leaves the row silently STALE (the reconciler compares the mutated array with itself and Keeps) | bug | the owner's kolt find (2026-09-06); reproduced under the ui_rows DOM stub on 757d3f4b; std, not user-land — `update` + `remove` is the documented idiom; B256 is the seam that lets the alias out, B257 the sibling |
...
| [B257](items/B257.md) | NEW — ASSIGNMENT from a live aggregate place does not copy: `b = a; a.push(4)` leaves `b` at 4, `r = q; q.x = 10` leaves `r.x` at 10 (lists and structs alike), while the binding form `mut f = e` clones — spec §6.1 names assignment first and its own example fails rewritten as one | bug | found under B255 (2026-09-06); miscompile class — a program that reads a copied aggregate after mutating the source computes the wrong value; `compute_clone_sites`' Assignment arm does `consider` the value, so one of its inputs declines |
...
| [M47](items/M47.md) | NEW — `List::remove`/`List::insert` are hand-written shift loops over the checked subscript helpers (`__at`/`__at_put` per element); lowered over the native `splice` (an intrinsic beside `pop`), with the bounds panic KEPT, they cost 8–29× less CPU on a 10k list and the same on a 6-element one | performance | the owner's question while migrating kolt (2026-09-06); measured in node 24 with `process.cpuUsage`, loadavg 0.03 |
