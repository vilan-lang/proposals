...
lane=$1; shift
cd "$W" || exit 1
git fetch -q origin 2>/dev/null || echo "fetch failed (agent locked?) — merging the LOCAL branch"
...
for spec in "$@"; do
  ...
  for t in $(echo "$spec" | grep -o -- '--test [A-Za-z0-9_]*' | awk '{print $2}'); do
    ...
    [ -f "crates/$crate/tests/$t.rs" ] || [ -f "crates/$crate/tests/$t/main.rs" ] || git cat-file -e "$lane:crates/$crate/tests/$t.rs" 2>/dev/null || git cat-file -e "$lane:crates/$crate/tests/$t/main.rs" 2>/dev/null || { echo "UNKNOWN TEST TARGET: -p $crate --test $t (no crates/$crate/tests/$t.rs in HEAD or on origin/$lane) — fix the spec"; exit 8; }
  done
done
git merge --no-ff --no-commit "$lane" >/dev/null 2>&1; echo "merge exit=$? (non-zero = conflicts to resolve)"
python3 "$S/changelog_union.py" "$lane" || { echo "CHANGELOG needs a hand"; exit 2; }
