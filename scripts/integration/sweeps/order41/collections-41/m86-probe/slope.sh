#!/usr/bin/env bash
# Usage: slope.sh <variant> [low] [high]  -> per-push Ir slope under node --jitless
set -euo pipefail
export PATH="$HOME/.cargo/bin:$HOME/.nvm/versions/node/v24.2.0/bin:$PATH"
cd "$(dirname "$0")"
variant=$1; low=${2:-20}; high=${3:-120}
ir() {
  local out=cg.$variant.$1.out
  M86_VARIANT=$variant M86_PUSHES=$1 valgrind --tool=callgrind --callgrind-out-file=$out node --jitless harness.js >/dev/null 2>cg.$variant.$1.err
